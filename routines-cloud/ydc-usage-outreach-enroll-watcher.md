...
Step 4: A "go" was found. Open the repository file .claude/skills/ydc-usage-outreach-daily/SKILL.md and follow its ENROLL MODE instructions exactly. Enroll mode reads the pending file from Drive, parses the reply for exclusions/inclusions, performs the Apollo enrollment via the Apollo connector, creates the daily-enrolled marker in Drive, and posts the Slack confirmation. Do not duplicate the enroll logic here — follow the file. Respect the file's cloud write boundary (no Salesforce writes). If the file cannot be read, exit without enrolling and post a one-line error notice in the same Slack thread.
```
