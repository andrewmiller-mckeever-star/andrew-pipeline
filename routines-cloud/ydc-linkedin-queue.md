...
If $APOLLO_API_KEY is unset or the call fails, post to C0B4LF2MPUJ: "<@{SLACK_USER_ID}> LinkedIn morning summary — could not reach Apollo API (check APOLLO_API_KEY env var in the cloud environment)." and stop.
...
Rules: plain text only, no markdown symbols. Always open with <@{SLACK_USER_ID}>. Keep it to one screen. Never hardcode the API key.
```
