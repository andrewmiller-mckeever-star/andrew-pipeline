...
description: >-
  CLOUD version for Claude Code Routines. Scans priority Salesforce accounts (Tier 1 and
  Tier 2.A) assigned to the current user via the hosted Salesforce connector, pulls
  Product_User__c API usage data, detects actionable signals (new activity, growth,
  multi-threading, sales gaps, risk), and produces a prioritized outreach list ranked by
  the Priority Lens. Also supports named account lookups — e.g. "check usage for Toggle AI".
  Use this skill whenever the user says "check usage", "usage monitor", "usage report",
...
- All Salesforce access goes through the account-level claude.ai Salesforce connector (read-only). Run every SOQL query with the connector tool `soqlQuery` (the tool name carries a platform suffix, e.g. `soqlQuery…platform_sobject_reads` — locate it by function-name suffix, never by a hardcoded `mcp__<uuid>__` prefix).
- No local MCP servers, no Salesforce DX CLI, no `~/.claude/settings.json`.
- WRITE BOUNDARY: this skill performs NO writes anywhere. It is read-only against Salesforce and prints its report directly in chat. Never write to Salesforce, Drive, Slack, or email.
...
**Salesforce connector unavailable entirely** → Report that the Salesforce connector is not connected/authorized and stop. Nothing in this skill works without it.
...
| Date | Change | Reason |
|------|--------|--------|
| 2026-07-17 | Cloud port created from skills/ydc-usage-monitor | Migration to Claude Code Routines: Salesforce DX MCP tool replaced by hosted Salesforce connector `soqlQuery` (located by function-name suffix), owner default moved to `SFDC_USER_ID` env var, explicit read-only write boundary added |
