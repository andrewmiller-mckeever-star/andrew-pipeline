...
- **Product focus: Search API is the primary lead.** All outreach should position Search API as the entry point (AI search infrastructure / grounding layer for agents and LLMs). **Exception: in financial services, fintech, banking, and investment accounts, the Finance Research API leads instead** (purpose-built financial index over SEC filings, fundamentals, market data, and financial news; supporting angle elsewhere). Vertical Index (aka Custom Index; same product, domain-specific or custom-built search indexes) and PRAG are supporting angles that show product depth when the account warrants it. Having these in the conversation shows we're not just a crawl API, but don't prescribe them in cold outreach; let them emerge when account research signals the need. Do NOT prescribe E2E solutions or complex multi-product stacks in cold outreach. De-emphasize AI Factory-As-A-Service (mention only when the account clearly lacks internal AI/engineering capacity). Do NOT reference ESL, Chat, Apex products, or the MCP Server in any prospect-facing output (the MCP Server is the technical access layer, internal context only).
- Product tiers for reference: APEX (Search API, Contents API, Research API, Finance Research API), E2E (end-to-end solutions), PRAG (publisher RAG), and AI Factory-As-A-Service. The Research API ships in five effort tiers (Lite, Standard, Deep, Exhaustive, Frontier); the Finance Research API in two (Deep, Exhaustive). Outreach should lead with Search API (Finance Research API for finance accounts), with Vertical Index and PRAG as supporting depth.
...
- **Always check Google Drive, Salesforce, and Slack for prior account context before starting any pipeline run.** Search Drive folder "Account Plans, Lists & Personalized Sequences/" for existing deliverables. Query Salesforce via the ydc-salesforce skill for structured CRM intelligence: account existence, opportunity history (open + closed), contacts, prospect replies ([Gong In] prefix), activity timeline, outbound sequences already run, and Databricks partnership signals. Salesforce is the primary CRM source. Search Slack channels (#api-gtm-team, #sales-team, #esl-api-sales, #competition, #enterprise-solutions) for supplemental informal context. All context should inform and be referenced in the account plan.
