...
4. **Product focus: Search API is the primary lead.** Vertical Index (aka Custom Index; same product) and PRAG are supporting angles that add depth and optionality when account signals warrant it. All three are valid outreach angles, but Search API always leads. Don't prescribe complex stacks in cold outreach. De-emphasize AI Factory. NEVER reference ESL/Chat/Apex in prospect-facing output.
...
- **Word counts:** Opener 100-150 words, follow-up 80-120, breakup 80-120
...
- **Technical details:** Uses real Chrome with existing profile (not Playwright test browser). Quill editor DOM injection for email bodies. Phase 0 UI dismissal for banners/modals. Safety check against Apollo's "Add personalized follow up" AI trap. Post-save verification. Results written to `_results.json`.
...
- **Key Apollo selectors (as of 2026-03-09):** Title = button containing "New Sequence" → click reveals `input[placeholder="Sequence name"]`. Step type menu = `div[role="menuitem"]`. Subject = `input[placeholder="Enter email subject"]`. Type dropdown = `div[role="combobox"]` → `div[role="option"]`. Body = `.ql-editor` (Quill). Save = button "Save changes".
